...
// ✅ v8 — Performance maximale : zéro animation coûteuse, swipe GPU-only
...
export default function OracleSelection({
  ...
  const containerW     = useRef(0); // ✅ largeur en cache — zéro reflow pendant le drag

  // ✅ Orb refs — mis à jour directement sans setState
  ...
  // ✅ Mesure la largeur du container une seule fois (et au resize)
  useEffect(() => {
    const measure = () => {
  ...
  // ✅ Met à jour les orbs directement via DOM — zéro setState, zéro re-render
  const updateOrbs = useCallback((idx: number) => {
    const o = oracles[idx];
    if (!o) return;
    if (orb1Ref.current) {
      orb1Ref.current.style.background = `radial-gradient(circle, ${o.accent}, ${o.accentAlt})`;
    }
    if (orb2Ref.current) {
      orb2Ref.current.style.background = o.accentAlt;
    }
  }, []); // eslint-disable-line
  ...
  const goTo = useCallback((i: number, animate = true) => {
    ...
    track.style.transition = animate
      ...
      : 'none';
    // ✅ px absolus — pas de % qui force un recalc
  ...
  const onTouchMove = useCallback((e: React.TouchEvent) => {
    ...
    // ✅ Seuil plus strict — décision plus rapide, pas d'hésitation
    ...
    const W = containerW.current; // ✅ valeur en cache — zéro reflow
    const base = -(currentRef.current * W);
    const bounded =
      (currentRef.current === 0 && dx > 0) ||
      (currentRef.current === totalRef.current - 1 && dx < 0)
        ? dx * 0.25
        : dx;

    // ✅ translate3d en px — le GPU composite directement sans recalc CSS
  ...
  return (
    <>
      <style>{`
        ...
        .oracle-page-in {
          animation: oraclePageIn 0.25s ease forwards;
        }
        .oracle-img {
          opacity: 0;
        ...
        /* ✅ shimmer retiré du badge — économise un layer de composition */
        .badge-ribbon {
          position: relative;
          display: inline-flex;
          align-items: center;
        ...
        /* ✅ Halo statique — remplace l'animation CSS coûteuse */
        .oracle-halo {
          border-radius: 50%;
          pointer-events: none;
        }
        /* ✅ Orbs : transition sur background uniquement, pas de filter animé */
        .oracle-orb {
      ...
      >
        {/*
          ✅ ÉTOILES : SVG statique sans <animate> — économise 50 animations SMIL
          L'effet scintillement est obtenu via CSS animation sur des groupes,
          pas sur chaque étoile individuellement
        */}
        <svg
        ...
        >
          ...
          `}</style>
          {/* ✅ 3 groupes de ~17 étoiles = 3 animations CSS au lieu de 50 */}
          <g className="star-g1">
        ...
        {/*
          ✅ ORBS : filter:blur retiré — remplacé par un gradient radial doux.
          blur() sur un élément fixed force un repaint de toute la page à chaque frame.
          Un radial-gradient donne un effet similaire sans aucun coût GPU.
        */}
        <div
          ref={orb1Ref}
          ...
          style={{
            ...
            background: `radial-gradient(circle, ${oracles[0].accent}, ${oracles[0].accentAlt})`,
            opacity: 0.07,
            zIndex: 0,
          }}
        />
        <div
          ...
          style={{
            ...
            background: oracles[0].accentAlt,
            opacity: 0.05,
            zIndex: 0,
          }}
        />
        ...
        <div
          ...
          style={{
            ...
            overflow: 'hidden',
            flexShrink: 0,
            // ✅ pan-y + manipulation = le navigateur sait immédiatement quoi faire
        ...
        >
          <div
            ...
            style={{
              display: 'flex',
              height: '100%',
              // ✅ px au lieu de % — cohérent avec les updates DOM direct
              transform: `translate3d(${-(current * (containerW.current || window.innerWidth))}px, 0, 0)`,
            }}
          >
            {oracles.map((oracle, idx) => {
              const ba = badgeAccent[oracle.id];
              // ✅ Rendu simplifié pour les cartes non-adjacentes
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    // ✅ contain strict — isole le layout/paint de chaque carte
                ...
                >
                  <div
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      overflow: 'visible',
                      // ✅ box-shadow simplifié — moins de layers
                      boxShadow: `0 8px 40px ${oracle.glowColor}, 0 2px 8px rgba(0,0,0,0.5)`,
                      // ✅ active state via CSS, pas via onMouseDown
                  ...
                  >
                    ...
                    >
                      {/*
                        ✅ Halo statique uniquement sur la carte active — pas d'animation CSS.
                        Effet visuel obtenu via opacity et box-shadow, zéro coût perf.
                      */}
                      {idx === current && (
                        <div className="oracle-halo" style={{
                      ...
                      {/* ✅ Seules les images proches sont chargées eagerly */}
                      <img
                        src={oracle.image}
